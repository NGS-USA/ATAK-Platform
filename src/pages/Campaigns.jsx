...
export default function Campaigns() {
  ...
  const [form, setForm] = useState({ name: "", description: "", status: "Active", start_date: "", map: "", faction: "" });
  ...
  const openEdit = (c) => {
    setEditing(c);
    setForm({ name: c.name, description: c.description || "", status: c.status, start_date: c.start_date || "", map: c.map || "", faction: c.faction || "" });
    setShowForm(true);
  };
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.5rem" }}>
        ...
        {canEdit && (
          <button onClick={() => { setShowForm(true); setEditing(null); setForm({ name: "", description: "", status: "Active", total_operations: 0, completed_operations: 0, start_date: "", map: "", faction: "" }); }} style={{ display: "flex", alignItems: "center", gap: "6px", background: "var(--accent)", color: "#000", border: "none", borderRadius: "8px", padding: "8px 16px", cursor: "pointer", fontWeight: 600, fontSize: "0.875rem" }}>
        ...
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "12px", padding: "1.5rem", marginBottom: "1.5rem" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
            ...
            <div>
              ...
              <select value={form.status} onChange={e => setForm({ ...form, status: e.target.value })} style={{ width: "100%", background: "var(--bg-secondary)", border: "1px solid var(--border)", borderRadius: "6px", padding: "8px 10px", color: "var(--text-primary)", fontSize: "0.875rem" }}>
                ...
                <div style={{ padding: "1.25rem 1.5rem" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "8px" }}>
                    ...
                    <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                       ...
                       <button onClick={() => setExpanded({ ...expanded, [c.id]: !open })} style={{ background: open ? "var(--accent)20" : "var(--bg-secondary)", border: `1px solid ${open ? "var(--accent)" : "var(--border)"}`, cursor: "pointer", color: open ? "var(--accent)" : "var(--text-secondary)", borderRadius: "8px", padding: "8px 12px", display: "flex", alignItems: "center", gap: "6px", fontWeight: 600, fontSize: "0.85rem", transition: "all 0.2s" }}>
                         {open ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
                         <span>{open ? "Hide" : "Details"}</span>
                       </button>
                     </div>
                  </div>
                  {c.description && <div style={{ color: "var(--text-secondary)", fontSize: "0.875rem", marginBottom: "12px", wordBreak: "break-word", overflowWrap: "break-word", maxWidth: "100%", overflow: "hidden" }} className="prose prose-sm prose-invert" dangerouslySetInnerHTML={{ __html: c.description }} />}
                              ...
                              return (
                                <div key={e.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 10px", background: "var(--bg-card)", borderRadius: "6px", border: "1px solid var(--border)" }}>
                                  ...
                                  <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                                    ...
                                    <span style={{ fontSize: "0.7rem", padding: "2px 8px", borderRadius: "4px", fontWeight: 600, background: hasAAR ? "#4ade8020" : "#94a3b820", color: hasAAR ? "#4ade80" : "#94a3b8" }}>{hasAAR ? "AAR Filed" : "Pending"}</span>
                                  </div>
                                </div>
                              );
                            })}
